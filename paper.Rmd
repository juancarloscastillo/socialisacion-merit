...
Because the sample has a hierarchical structure (students nested in schools), multilevel regressions were estimated to contrast the hypotheses following the  steps recommended to estimate and report using this type of models [@aguinis_bestpractice_2013a]. Since the two indicators of student perception of meritocracy are ordinal variables, we consideded estimations according to this type of measurement [@arfan_ordinal_2017a] [^2]. The analysis was carried out with statistical software `R` version 4.0.3 "Bunny-Wunnies Freak Out."

[^2]: In multilevel ordinal models, the dependent variable represents the calculation of the cumulative probability that a student will respond up to $C$ in the meritocracy perception indicators,  $Y_{cij}$ being an ordered categorical response of a student $i^{th}$ in a school (cluster) $j^{th}$ with $C$ ordered categories, coded as $C = 1,2,3,4$. This probability is calculated based on the intercept for each category $a_c$, the coefficients $\gamma_1$ and $\gamma_2$for the perception of meritocracy of the parents and the sense of justice in grading respectively, $\gamma_n$for the control variables, $u_{0j}$as the error term for a school $j$and $e_{ij}$as the estimation error for the individual $i$in a school $j$. The library `clmm2` of R. was used for the estimation.
