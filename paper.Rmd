...
<!-- Primera versión de introducción -->
...
# Paquetes base. pacman para manejar paquetes de R y tinytex para renderizar documento a pdf.
if (!require("pacman")) install.packages("pacman")  #si falta pacman, instalar
if (!require("tinytex")) install.packages("tinytex")#si falta tinytex, instalar
pacman::p_load(knitr, kableExtra, dplyr, haven) # librerias
...
# Crear tabla
tab01 <- read.csv(file = "input/prereg/table01.csv",header = 1,sep = ";",encoding = "UTF-8") # Leer .csv con la estructura de la tabla.
cnames <- c("Variable","Pregunta", "Categorías de respuesta") # Nombre de las columnas
kable(tab01,table_format,booktabs = T, linesep = "",col.names = cnames, caption = cap1) %>% # Tabla
    kable_styling(
    full_width = fwidth,
    latex_options = c("hold_position"), # Fijar tabla en esta sección del documento
    position = "center",
    font_size = format_size) %>%
  column_spec(column = 1, width = "2 cm", ) %>%
  column_spec(column = 2,width = "7 cm") %>%
  column_spec(column = 3,width = "4 cm") %>%
  collapse_rows(columns = 1:2,valign = "middle") %>% # Colapsar filas con mismo contenido
      gsub("font-size: initial !important;", # Forzar tamaño de fuente en el título de la tabla
...
# Crear tabla
tab02 <- read.csv(file = "input/prereg/table02.csv",header = 1,sep = ";",encoding = "UTF-8") # Leer .csv con la estructura de la tabla
kable(tab02,table_format,booktabs = T, linesep = "",col.names =  c("Variable","Pregunta","Categorías de respuesta"), caption = cap2) %>% # Tabla
  kable_styling(
    full_width = fwidth,
    latex_options = c("hold_position"), # Fijar posición de la tabla en esta sección del documento
    position = "center",
    font_size = format_size) %>%
...
# Crear tabla
tab03 <- read.csv(file = "input/prereg/table03.csv",header = 1,sep = ";",encoding = "UTF-8") # Leer .csv con la estructura de la tabla
kable(tab03,table_format,booktabs = T, linesep = "",col.names =  c("Variable","Pregunta","Categorías de respuesta"), caption = cap3) %>% # Tabla
  kable_styling(
    full_width = fwidth,
    latex_options = c("hold_position"), # Fijar tabla en esta sección del documento
    position = "center",
  ...
  column_spec(column = 3,width = "4 cm") %>%
  collapse_rows(columns = 1:2,valign = "middle")  %>% # Colapsar filas con el mismo contenido
      gsub("font-size: initial !important;", # Forzar tamaño fuente título tablas
         "font-size: 20 !important;", 
         .)
```
...
basereg <- ap_est %>% select(perc_trabajo_duro_factor_est,
                             ...
                             univ_esc,
                             # rbd_est
) %>% na.omit(basereg) %>% select(-recompensa_factor_est,-resp_prof_factor) # Estas dos variables son parte de análisis exploratorios. Se seleccionaron para que el N fuera homogeneo a través de todos los análisis, sin emabrgo, no son incluídas dentro de los análisis principales de hipótesis.
...
knitr::kable(data.frame(image='![](output/tables/desc01.png){width=width_table}'),    # generar tabla
...
if (!require("pacman")) install.packages("pacman")  #si falta pacman, instalar
if (!require("tinytex")) install.packages("tinytex")#si falta tinytex, instalar
pacman::p_load(knitr, kableExtra, dplyr, haven, texreg, sjPlot, lme4, ordinal) # librerias
knitr::opts_chunk$set(warning = FALSE,  # mensaje de warning
                      message = FALSE,  # mensajes/avisos de librerias  
                      cache = FALSE,    # cache de los chunks,usar analisis pesados
                      out.width = '85%',# largo de imagen en %
...
knitreg(list(getah.or1,getah.or3,getah.or4,getah.or5,getah.or6,getah.or7,getah.or8),
        caption = paste("(\\#tab:getah)",cap1), # Se agrega key para que se pueda referenciar
        caption.above=TRUE,
        omit.coef=omit_coef,
