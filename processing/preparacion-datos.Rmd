...
# Trabajo duro
est_proc$perc_trabajo_duro_factor <- factor(est_proc$perc_esfuerzo, levels = c(1,2,3,4), labels = c("Nada importante", "Algo importante", "Importante", "Muy importante"))
...
Se construyen cuatro variables nuevas. La primera es el ingreso a partir de las marcas de clase de los tramos de ingreso. La segunda es el ingreso per capita. La tercera se construye una variable en tramos tipo factor y la cuarta serán los quintiles de ingreso per capita.
...
#ap_proc$quintiles_ingresos_pc <- quantile(ap_proc$ingresos_pc, prob=seq(0,1,1/5), na.rm = T)
...
ap_est$univ<- ifelse(ap_est$educ_ap==4,1,0)
 ap_est <- ap_est %>%
     group_by(rbd_est) %>%
     mutate(univ_esc = mean(univ, na.rm = T))
...
# ---- Descriptivos nuevas variables ----
frq(ap_est$univ)
summary(ap_est$univ_esc)

#---- Etiquetado nuevas variables ----
ap_est$univ <- set_label(x = ap_est$univ, label = "Educacion terciaria apoderados") # Etiquetado variable

ap_est$univ <- set_labels(ap_est$univ,
            labels=c( "Universitario"=1,
                      "No universitario"=0)) #Etiquetado categorias

ap_est$univ_esc <- set_label(x = ap_est$univ_esc, label = "Porcentaje ed. terciaria por escuela")
...
```{r}
# Centrado a la media grupal

## Percepción Esfuerzo apoderado
## Percepción Trabajo Duro apoderado
## Sentido justicia indirecto Ln
## Sentido justicia directo
## Ingresos tramos (?)
## Region estudiante
## Dependencia estudiante
## Posición Política estudiante
## Genero estudiante
## Libros Hogar estudiante
