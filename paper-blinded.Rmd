...
```{r plotgetah, eval=FALSE, include=FALSE}
# Funciones para renderizar tablas tanto en HTML como en pdf
if (!require("pacman")) install.packages("pacman")  #si falta pacman, instalar
if (!require("tinytex")) install.packages("tinytex")#si falta tinytex, instalar
pacman::p_load(knitr, kableExtra, dplyr, haven, texreg, sjPlot, lme4, ordinal, webshot,ggplot2) # librerias
...
basereg$perc_tduro_bi_ap <- factor(basereg$perc_tduro_bi_ap,labels = c("Not important","Is important"))

# getah.or6 <- ordinal::clmm(formula(paste("perc_trabajo_duro_factor_est~",paste(c("perc_tduro_bi_ap","sj_indirect_ln","sj_direct_factor",con,ref),collapse="+"))), basereg, nAGQ = 10)
# 
# getah.or8 <- ordinal::clmm(formula(paste("perc_trabajo_duro_factor_est~",paste(c("perc_tduro_bi_ap","sj_indirect_ln","sj_direct_factor", con, "perc_tduro_bi_ap*sj_direct_factor","nse_grupo",ref),collapse="+"))), basereg)

getah.or6 <- lmer(formula(paste("as.numeric(perc_trabajo_duro_factor_est)~",paste(c("perc_tduro_bi_ap","sj_indirect_ln","sj_direct_factor","nse_grupo",con,ref),collapse="+"))), basereg)

getah.or8 <- lmer(formula(paste("as.numeric(perc_trabajo_duro_factor_est)~",paste(c("perc_tduro_bi_ap","sj_indirect_ln","sj_direct_factor", "perc_tduro_bi_ap*sj_direct_factor",con,ref),collapse="+"))), basereg)

rm_plot <- c(
  # "Not important|Somewhat important",
  # "Somewhat important|Important",
  # "Important|Very important",
  "quintiles_ingresos_pc_factorDon't know/No response",
  "libros_hogar_factor_apBetween 11 and 25 books",
  "libros_hogar_factor_apBetween 26 and 100 book",
  "libros_hogar_factor_apBetween 101 and 200 books",
  "libros_hogar_factor_apBetween 201 and 500 books",
  "libros_hogar_factor_apMore than 500 books",
  # "univ",
  "pos_pol_factor_apCenter Right",
  "pos_pol_factor_apCenter",
  "pos_pol_factor_apCenter Left",
  "pos_pol_factor_apLeft",
  "pos_pol_factor_apIndependent",
  "pos_pol_factor_apNone",
  "pos_pol_factor_apNa/Nr",
  "pos_pol_factor_estCenter Right",
  "pos_pol_factor_estCenter",
  "pos_pol_factor_estCenter Left",
  "pos_pol_factor_estLeft",
  "pos_pol_factor_estIndependent",
  "pos_pol_factor_estNone",
  "pos_pol_factor_estNa/Nr",
  "genero_factor_estMujer",
  "genero_factor_estOtro",
  "dependencia_factor_apPart. Subvencionado",
  "dependencia_factor_apPart. Privado")


coef_names <- c(
    "Hard work is important (ref: Not important)",
  "Sense of Justice in grades - Indirect (ln)",
  "Less than I deserve (ref: The ones I deserve)",
  "More than I deserve ",
...
# plotgetah<- 
# plot_model(getah.or8,rm.terms = rm_plot,show.intercept = F,
#            show.p = T, show.values = T,
#            # order.terms = c(2,3,4,1,5,6,7,8,9,10,11,12),
#            axis.labels = rev(coef_names),
#            ) +
...
plotgetah<- 
plot_models(getah.or6,getah.or8,rm.terms = rm_plot,
            axis.labels = rev(coef_names),
            show.values = T,
            m.labels = c("Base", "Interactions"),
            legend.title = "Models") +
  geom_hline(yintercept = 0) +
  theme(legend.position = "bottom")  

ggplot2::ggsave(plot = plotgetah,device = "png",units = "cm",
                width = 1,height = 1,scale = 18,
                path =  "docs/presentacion-effort/images/",
                filename = "plotgetahead.png")
...
graph_interact <- 
plot_model(getah.or8,type =  "int") +
  xlab("Importance of hard work (parents)") +
  ylab("Predicted values: Importance of hard work (students)") +
  ggtitle(NULL)+
...
# 
# interplot::interplot(m = getah.or8, 
#                      var1 = "sj_direct_factor", 
#                      var2 = "perc_tduro_bi_ap")

# ggeffects::ggpredict(getah.or8,terms =c("perc_tduro_bi_ap","sj_direct_factor")) %>% plot()
# ggeffects::ggemmeans(getah.or8,terms =c("perc_tduro_bi_ap","sj_direct_factor")) %>% plot()

# graph_interact

ggplot2::ggsave(plot = graph_interact,device = "png",units = "cm",
                width = 1,height = 1,scale = 13,
                path =  "docs/presentacion-effort/images/",
                filename = "intergetahead.png")
```
...
```{r ploteffort, eval=FALSE, include=FALSE}
# Funciones para renderizar tablas tanto en HTML como en pdf
if (!require("pacman")) install.packages("pacman")  #si falta pacman, instalar
if (!require("tinytex")) install.packages("tinytex")#si falta tinytex, instalar
pacman::p_load(knitr, kableExtra, dplyr, haven, texreg, sjPlot, ggplot2, ordinal, webshot) # librerias
knitr::opts_chunk$set(warning = FALSE,  # mensaje de warning
                      message = FALSE,  # mensajes/avisos de librerias
                      cache = FALSE,    # cache de los chunks,usar analisis pesados
                      out.width = '85%',# largo de imagen en %
...
effort.or6 <- lme4::lmer(formula(paste("as.numeric(perc_esfuerzo_factor_est)~",paste(c("perc_esfuerzo_bi_ap","sj_indirect_ln","sj_direct_factor", con,ref),collapse="+"))), basereg)
effort.or8 <- lme4::lmer(formula(paste("as.numeric(perc_esfuerzo_factor_est)~",paste(c("perc_esfuerzo_bi_ap","sj_indirect_ln","sj_direct_factor", con, "perc_esfuerzo_bi_ap*sj_direct_factor","nse_grupo",ref),collapse="+"))), basereg)
...
coef_names <- c(
  "Effort is rewarded (ref: Effort is not rewarded)",
  "Sense of Justice in grades - Indirect (ln)",
  "Less than I deserve (ref: The ones I deserve)",
  "More than I deserve",
  "Quintile 2 (ref: Quintile 1)",
...
# ploteffort<- 
# plot_model(effort.or8,rm.terms = rm_plot,show.intercept = F,
#            show.p = T, show.values = T,
#            order.terms = c(2,3,4,1,5,6,7,8,9,10,11,12),
#            axis.labels = rev(coef_names),
#            ) +
...
ploteffort<- 
plot_models(effort.or6,effort.or8,rm.terms = rm_plot,
            axis.labels = rev(coef_names),show.values = T,
            m.labels = c("Individual", "Interactions"),
            legend.title = "Models") +
  geom_hline(yintercept = 1) +
  theme(legend.position = "bottom")


ggplot2::ggsave(plot = ploteffort,device = "png",units = "cm",
                width = 1,height = 1,scale = 19,
                path =  "presentacion-effort/images/",
                filename = "ploteffort.png")
# Elaborar modelo de interacción
effort.or7 <- ordinal::clmm(formula(paste("perc_esfuerzo_factor_est~",paste(c("perc_esfuerzo_bi_ap","sj_indirect_ln","sj_direct_factor", con, "perc_esfuerzo_bi_ap*sj_direct_factor",ref),collapse="+"))), basereg)
