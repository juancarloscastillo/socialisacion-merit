...
basecor<- ap_est %>% select(perc_trabajo_duro_est,perc_trabajo_duro_ap,perc_esfuerzo_est,perc_esfuerzo_ap,prom_percmer_est,prom_percmer_ap,sj_indirect_ln,sj_direct,ingresos_pc_esc, recompensa_est, recompensa_ap, prom_obt, prom_mer)
...
rownames(cormat) <-c(
    ...
    "L. Promedio obtenido",
    "M. Promedio merecido")
colnames(cormat) <-c("(A)", "(B)","(C)","(D)","(E)","(F)", "(G)", "(H)", "(I)", "(J)", "(K)", "(L)", "(M)")
corrplot(
cormat,
  method = "color",
  type = "upper",
...
basecor<- ap_est %>% select(ingresos_esc, ingresos_pc_esc, heterogen_esc, univ_esc, dependencia_ap, porc_muj_esc_ap, porc_muj_esc_est)
cormat=cor(basecor, use = "complete.obs")
#save
...
rownames(cormat) <-c(
    ...
    "F. Porcentaje de mujeres apoderadas en la escuela",
    "G. Porcentaje de mujeres estudiantes en la escuela")
...
En esta segunda versión de los resultados se mantendrá la división de sin centrar/CWC, pero esta vez la secuencia será distinta. Existirá un modelo por cada introducción de variables principales, quedando de la siguiente manera: Modelo nulo - Modelo B1 + Modelo B2 + Modelo B3 + Modelo B4 + Modelo con controles e interacción + Modelo con controles y otra interacción. Además, se testeará el uso del rbd reportado por apoderados, asi como también la introducción o no de la variable recompensa.
...
#### Modelo 1: B1 (Percepción meritocrática apoderados) promedio indicadores sin centrar
```{r}
# Percepción meritocracia
mmb1_p <- lmer(prom_percmer_est ~ 1 + prom_percmer_ap_cwc + 
                 (1|rbd_est), data = ap_est)
```

#### Modelo 2:  B2 (Sentidos de justicia) con promedio indicadores CWC
```{r}
# Percepción meritocracia
mmb2_p <- lmer(prom_percmer_est ~ 1 + sj_indirect_ln_cwc + sj_direct_cwc
              + (1|rbd_est), data = ap_est)
```

#### Modelo 3: B3 (Quintiles de ingreso) con promedio indicadores CWC
```{r}
# Percepción meritocracia
mmb3_p <- lmer(prom_percmer_est ~ 1 + quintiles_ingresos_pc_factor
                + (1|rbd_est), data = ap_est)
```

#### Modelo 4: B1+B2+B3 con promedio indicadores CWC
```{r}
# Percepción meritocracia
mmb1b2b3_p <- lmer(prom_percmer_est ~ prom_percmer_ap_cwc + 
                 sj_indirect_ln_cwc + 
                 sj_direct_cwc + 
...
#### Modelo 5: Controles e I1 con promedio indicadores CWC
```{r}
# Percepción meritocracia
mmin1_p <- lmer(prom_percmer_est ~ prom_percmer_ap_cwc +
                 sj_indirect_ln_cwc + 
                 sj_direct_cwc + 
                 quintiles_ingresos_pc_factor +
                 prom_obt_cwc +
...
#### Modelo 6: Controles e I2 con promedio indicadores CWC
```{r}
# Percepción meritocracia
mmin2_p <- lmer(prom_percmer_est ~ prom_percmer_ap_cwc +
                 sj_indirect_ln_cwc + 
                 sj_direct_cwc + 
                 quintiles_ingresos_pc_factor + 
                 prom_obt_cwc +
...
### Tabla de modelos con promedio de indicadores CWC
```{r Tabla de modelos con promedio de indicadores CWC, results='asis'}

htmlreg(list(mmn_p, mmb1_p, mmb2_p, mmb3_p, mmb1b2b3_p, mmin1_p, mmin2_p), custom.model.names = c("Nulo", "B1", "B2", "B3", "B1+B2+B3", "I1", "I2"), star.symbol = "\\*", center = T, custom.note = " <div style='text-align: justify'> $***p <$ 0.001, $**p <$ 0.01, $*p <$ 0.05 <br>" , caption.above = T, caption = "Socialización de la percepción meritocratica: Promedio percepción meritocrática estudiantes (CWC)")
```
...
mml1_p_cwc3 <- lmer(prom_percmer_est ~ 1 + resp_prof + (1|rbd_est), data = ap_est)
mml1_p_cwc4 <- lmer(prom_percmer_est ~ 1 + quintiles_ingresos_pc_factor + (1|rbd_est), data = ap_est)

htmlreg(list(mmn_p_cwc, mml1_p_cwc, mml1_p_cwc2, mml1_p_cwc3, mml1_p_cwc4), custom.model.names = c("Nulo", "Model 1", "Model 2", "Model 3", "Model 4"), star.symbol = "\\*", center = T, custom.note = " <div style='text-align: justify'> $***p <$ 0.001, $**p <$ 0.01, $*p <$ 0.05 <br>" , caption.above = T, caption = "Socialización de la percepción meritocratica: Promedio percepción meritocrática estudiantes (CWC)")
```

```{r, results='asis'}
# Percepción meritocracia modelo nulo

basereg <- ap_est %>% select(prom_percmer_est, rbd_est, prom_percmer_ap_cwc, sj_direct_cwc, quintiles_ingresos_pc_factor, recompensa_est_cwc, resp_prof_cwc, pos_pol_ap)

basereg <- na.omit(basereg)

mmn_p_cwc <- lmer(prom_percmer_est ~ 1 + (1|rbd_est), data = basereg)

mml1_p_cwc <- lmer(prom_percmer_est ~ 1 + 
                     prom_percmer_ap_cwc + 
                     quintiles_ingresos_pc_factor + 
                     pos_pol_ap +  (1|rbd_est), data = basereg)
